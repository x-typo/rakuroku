...
struct MediaDetailView: View {
    ...
    private func mediaContent(_ media: MediaDetails) -> some View {
        ...
        .sheet(isPresented: $showCandidateSheet) { candidateSheet(media) }
        .sheet(isPresented: $showOverrideSheet) { overrideSheet(media) }
    ...
    @ViewBuilder
    private func watchSection(_ media: MediaDetails) -> some View {
        let currentProgress = userEntry?.progress ?? 0
        let nextEp = currentProgress + 1
        let allWatched = media.episodes.map { currentProgress >= $0 } ?? false
        let canWatch = authStore.isAuthenticated && userEntry != nil && !allWatched

        VStack(alignment: .leading, spacing: 8) {
            if canWatch {
                Button {
                    Task { await handleWatch(media) }
                } label: {
                    HStack(spacing: 8) {
                        if watchLoading {
                ...
                HStack(spacing: 8) {
                    Button {
                        safariURL = URL(string: "https://animekai.to/home")
                    } label: {
                        Text("AnimeKai")
    ...
    private func handleWatch(_ media: MediaDetails) async {
    ...
    @ViewBuilder
    private func candidateSheet(_ media: MediaDetails) -> some View {
        let nextEp = (userEntry?.progress ?? 0) + 1
                ...
                VStack(spacing: 4) {
                    ForEach(watchCandidates) { candidate in
                        Button {
                            ...
                            safariURL = AnimeKaiResolver.buildEpisodeURL(watchPath: candidate.watchPath, episode: nextEp)
                        } label: {
    ...
    @ViewBuilder
    private func overrideSheet(_ media: MediaDetails) -> some View {
        VStack(spacing: 16) {
